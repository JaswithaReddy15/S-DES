...
def ls(key,n):
    # n=1: performs circular left rotate once
    # n=2: performs circular left rotate twice
    # applying circular left rotate to first half
    for j in range(n):
        temp1=key[0]
        for i in range(4):
            key[i]=key[i+1]
        key[4]=temp1
    # applying circular left rotate to second half
    for j in range(n):
        temp2=key[5]
        for i in range(5,9):
            key[i]=key[i+1]
...
if __name__=='__main__':
    count=1 # to check if key is valid or not.
    inpkey=[] # initializing key list
    k=input(("Enter a 10 bit key in binary: "))
